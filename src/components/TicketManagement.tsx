...
import { Plus, LogOut, Edit2, Trash2, Ticket, X } from "lucide-react";
            ...
            {tickets.map((ticket) => (
              <div key={ticket.id} className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
                <div className="flex justify-between items-start gap-2 mb-3 sm:mb-4">
                  <h3 className="text-lg sm:text-xl font-bold text-gray-900 break-words flex-1">
                    {ticket.title}
                  </h3>
                  <span
...
function TicketModal({ ticket, onClose, onSave }: TicketModalProps) {
      ...
      <div className="bg-white rounded-xl p-4 sm:p-6 lg:p-8 max-w-md w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
            ...
            <button
              onClick={onClose}
              className="w-full py-2.5 sm:py-3 bg-gray-200 text-gray-800 rounded-lg font-semibold hover:bg-gray-300 transition text-sm sm:text-base"
            >
              Cancel
            </button>
          </div>
